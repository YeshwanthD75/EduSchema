...
def create_gui(connection):
    ...
    def view_all(table_name):
        records = get_all_records(connection, table_name)
    ...
    def delete_instructor():
        name = instructor_name_entry.get()
        delete_record(connection, "instructors", "name", name)

    ttk.Button(instructors_frame, text="Add Instructor", command=add_instructor).grid(row=4, column=0, columnspan=2, pady=10)
    ttk.Button(instructors_frame, text="Delete Instructor", command=delete_instructor).grid(row=5, column=0, columnspan=2, pady=10)

    ttk.Label(instructors_frame, text="Search by Name:").grid(row=6, column=0, padx=10, pady=10)
    search_name_entry = ttk.Entry(instructors_frame)
    search_name_entry.grid(row=6, column=1, padx=10, pady=10)

    def search_instructors():
        name = search_name_entry.get()
        display_search_results(connection, name)

    ttk.Button(instructors_frame, text="Search", command=search_instructors).grid(row=7, column=0, columnspan=2, pady=10)

    def sort_asc():
        display_sorted_results(connection, 'ASC')

    def sort_desc():
        display_sorted_results(connection, 'DESC')

    ttk.Button(instructors_frame, text="Sort Ascending", command=sort_asc).grid(row=8, column=0, padx=10, pady=10)
    ttk.Button(instructors_frame, text="Sort Descending", command=sort_desc).grid(row=8, column=1, padx=10, pady=10)
    
    ttk.Button(instructors_frame, text="View All Instructors", command=lambda: view_all("instructors")).grid(row=9, column=0, columnspan=2, pady=10)
    ttk.Button(instructors_frame, text="Backup Instructors Data", command=lambda: backup_data("instructors")).grid(row=10, column=0, columnspan=2, pady=10)
    ...
    def delete_course():
        name = course_name_entry.get()
        delete_record(connection, "courses", "name", name)

    ttk.Button(courses_frame, text="Add Course", command=add_course).grid(row=4, column=0, columnspan=2, pady=10)
    ttk.Button(courses_frame, text="Delete Course", command=delete_course).grid(row=5, column=0, columnspan=2, pady=10)
    ...
    def delete_student():
        name = student_name_entry.get()
        delete_record(connection, "students", "name", name)

    ttk.Button(students_frame, text="Add Student", command=add_student).grid(row=3, column=0, columnspan=2, pady=10)
    ttk.Button(students_frame, text="Delete Student", command=delete_student).grid(row=4, column=0, columnspan=2, pady=10)
    ...
    def add_enrollment():
        ...
        insert_enrollment(connection, student_id, course_id)

    def delete_enrollment():
        student_id = enrollment_student_id_entry.get()
        course_id = enrollment_course_id_entry.get()
        delete_enrollment(connection, student_id, course_id)

    ttk.Button(enrollments_frame, text="Add Enrollment", command=add_enrollment).grid(row=2, column=0, columnspan=2, pady=10)
    ttk.Button(enrollments_frame, text="Delete Enrollment", command=delete_enrollment).grid(row=3, column=0, columnspan=2, pady=10)
    
    ttk.Button(enrollments_frame, text="View All Enrollments", command=lambda: view_all("enrollments")).grid(row=4, column=0, columnspan=2, pady=10)
    ttk.Button(enrollments_frame, text="Backup Enrollments Data", command=lambda: backup_data("enrollments")).grid(row=5, column=0, columnspan=2, pady=10)
    ...
    def delete_assessment():
        name = assessment_name_entry.get()
        delete_record(connection, "assessments", "name", name)

    ttk.Button(assessments_frame, text="Add Assessment", command=add_assessment).grid(row=3, column=0, columnspan=2, pady=10)
    ttk.Button(assessments_frame, text="Delete Assessment", command=delete_assessment).grid(row=4, column=0, columnspan=2, pady=10)
...
def get_all_records(connection, table_name):
    ...
    try:
        ...
        print(f"Error fetching records from {table_name}: {err}")
        return []
...
def backup_table_data(connection, table_name):
    ...
    try:
        ...
        for record in records:
            ...
            cursor.execute(f"INSERT INTO backup_{table_name} ({columns}) VALUES ({values})")
        connection.commit()
        print(f"Backup of {table_name} completed successfully.")
    except mysql.connector.Error as err:
        print(f"Error backing up {table_name}: {err}")

# Function to delete a record from a table
def delete_table_record(connection, table_name, record_id):
    cursor = connection.cursor()
    try:
        cursor.execute(f"DELETE FROM {table_name} WHERE {table_name[:-1]}_id = %s", (record_id,))
        connection.commit()
        print(f"Record with ID {record_id} from {table_name} deleted successfully.")
    except mysql.connector.Error as err:
        print(f"Error deleting record from {table_name}: {err}")
